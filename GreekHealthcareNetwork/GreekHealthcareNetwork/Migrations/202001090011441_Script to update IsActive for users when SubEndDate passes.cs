    ...
    public partial class ScripttoupdateIsActiveforuserswhenSubEndDatepasses : DbMigration
    {
			...
			Sql(@"USE [GreekHealthcareNetwork]
					...
					EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'IsActiveCheckAndUpdate', 
							...
							@command=N'UPDATE AspNetUsers
										SET IsActive = 0
										WHERE SubscriptionEndDate != ''0001-01-01'' AND SubscriptionEndDate < GETDATE() AND IsActive = 1;', 
        ...
        public override void Down()
        {
			Sql(@"USE [GreekHealthcareNetwork]
				...
				/****** Object:  Job [UpdateUsersIsActive]    Script Date: 09-Jan-20 2:16:11 ******/
				EXEC msdb.dbo.sp_delete_job @job_id=N'305e559f-8bff-4b69-bcee-c572755617c6', @delete_unused_schedule=1
				GO");
        }
    }
}
